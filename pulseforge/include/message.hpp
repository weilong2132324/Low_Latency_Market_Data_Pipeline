/*
 * message.hpp - fixed-size TickMessage and validation.
 *
...
#ifndef PULSEFORGE_MESSAGE_HPP
#define PULSEFORGE_MESSAGE_HPP

#include <cstddef>
#include <cstdint>

/* Known flag bits. Unknown/reserved bits must be zero (validated). */
enum TickFlag : uint32_t {
 ...
 *
 * Compilers insert padding to satisfy each member's alignment; neither
 * C nor C++ guarantees field offsets, so never assume this layout for a
 * wire format. This lab deliberately uses the in-memory struct as the
 * local "wire" format for simplicity.
 *
 * ENDIANNESS: on this lab everything runs on one little-endian machine,
 * so we ignore byte order. Real protocols use explicit byte-order
 * conversions and a versioned, padded wire schema - out of scope here.
 */
constexpr size_t TICK_MESSAGE_EXPECTED_SIZE = 40;

static_assert(sizeof(TickMessage) == TICK_MESSAGE_EXPECTED_SIZE,
 ...
 * Returns true if the message is well-formed. Takes a reference: there
 * is no "null message" in C++ - pass a zero-initialized message to test
 * the degenerate case.
 */
inline bool message_is_valid(const TickMessage &msg) {
    if (msg.quantity == 0) {
    ...
    constexpr uint32_t kKnownFlags =
        TICK_FLAG_TRADE | TICK_FLAG_QUOTE | TICK_FLAG_LAST;
    if (msg.flags & ~kKnownFlags) {
        return false;
    }
    return true;
}

#endif /* PULSEFORGE_MESSAGE_HPP */
